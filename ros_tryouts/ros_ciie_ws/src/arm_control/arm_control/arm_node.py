...
class ArmNode(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "baud", int(env_baud) if env_baud.isdigit() else default_baud
        )
        ...
        if self.serial_enabled:
            ...
            self.get_logger().warn(
                f"Serial DESHABILITADO ({msg}). El nodo funcionará solo por tópicos."
            )

        self.get_logger().info(
            "ArmNode listo. Subs: /arm/command | Pubs: /arm/status, /arm/command_echo"
        )
    ...
    def _open_serial(self):
        while self.serial is None and self.serial_enabled:
            try:
                ...
                self._status(
                    f"No se pudo abrir {self.port}: {e}. Reintentando en 1s…", warn=True
                )
                time.sleep(1)
    ...
    def on_cmd(self, msg: String):
        ...
        if not cmd:
            self._status(
                f'Comando inválido: "{raw}". Válidos: {list(CMD_MAP.keys())}', warn=True
            )
            return
        ...
        if not self.serial_enabled:
            ...
            self._status(
                "Serial deshabilitado: ejecutando en modo dry-run (no se envía a Arduino).",
                warn=True,
            )
            return
